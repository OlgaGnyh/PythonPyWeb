...
class Author(models.Model):
    ...
    username = models.SlugField(verbose_name='Имя авторского аккаунта',
                                ...
                                unique=True,)
    email = models.EmailField(verbose_name='Адрес Электронной почны',
    ...
    phone_number = models.CharField(verbose_name='Телефон', max_length=12, validators=[phone_regex],
...
class Entry(models.Model):
    ...
    def __str__(self):
        # return f"Тег статьи: {self.tags.all().values_list('name')}, Автор: {self.author}"

        return f"Текст статьи: {self.text}, Автор: {self.author}"
